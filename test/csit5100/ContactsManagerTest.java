...
//	}
	...
	public void testAdd() {
		...
		cm.fName.setText("");
		cm.save.doClick();
		assertEquals("Empty first name Save", beforeContactsCnt+1, cm.contacts.size());
		
		cm.add.doClick();
		cm.fName.setText("test");
		cm.lName.setText("");
		cm.save.doClick();
		assertEquals("Empty last name Save", beforeContactsCnt+1, cm.contacts.size());
	}
	...
	public void testRemove() {
		...
		assertEquals("No Entry Selected", beforeContactsCnt+1, cm.contacts.size());
		
		cm.list.setSelectedIndex(0);
		
		cm.remove.doClick();
		assertEquals("Remove the first item", beforeContactsCnt, cm.contacts.size());
	}
	...
	public void testEdit(){
		...
		if(cm.contacts.size() > 0){
			...
			assertEquals("Check Selected edit item", "first Name"+"last name"+"test phone\n"+"test address\n", fName+lName+phoneNums+addresses);
